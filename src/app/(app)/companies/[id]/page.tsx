...
export default function CompanyDetailPage() {
  ...
  if (!company) return <div className="p-8 text-slate-400">Chargement...</div>;
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label>Pays</Label>
              <select className="w-full px-3 py-2 rounded-lg border border-slate-600 bg-slate-800 text-slate-50 text-sm disabled:opacity-60" value={form.country ?? "France"} onChange={(e) => set("country", e.target.value)} disabled={!isAdmin}>
      ...
      <Card>
        ...
        <CardContent>
          {company.agencies.length === 0 ? (
            <p className="text-sm text-slate-500">Aucune agence. L&apos;entreprise n&apos;a que son siege.</p>
          ) : (
            <div className="space-y-2">
              {company.agencies.map((a) => (
                <div key={a.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50 group">
                  <div>
                  ...
                  {isAdmin && (
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                      <button onClick={() => openEditAgency(a)} className="p-1.5 text-slate-400 hover:text-slate-100"><Pencil className="w-4 h-4" /></button>
                      <button onClick={() => deleteAgency(a)} className="p-1.5 text-slate-400 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  )}
                </div>
              ))}
