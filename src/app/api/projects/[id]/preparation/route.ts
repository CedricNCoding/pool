...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const project = await prisma.project.findFirst({
    where: { id },
    select: { id: true, title: true, requiredEpi: true, requiredCertifications: { select: { id: true, name: true } } },
  });
  ...
  const crew = techIds.map((tid) => {
    ...
    return { technician: techMap.get(tid), missingCerts, missingEpi, ok: missingCerts.length === 0 && missingEpi.length === 0 };
  });
